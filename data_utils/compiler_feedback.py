...
class STValidator:
    ...
    VALID_TYPES = [
        ...
        "BYTE", "WORD", "DWORD", "UINT", "UDINT", "SINT", "USINT",
        "DATE", "TOD", "DT", "ARRAY",
    ]
    ...
    def validate_syntax(code: str) -> ValidationResult:
        ...
        for open_kw, close_kw in STValidator.BLOCK_PAIRS:
            opens = sum(
                1 for line in lines
                if re.search(rf"\b{open_kw}\b", line.strip())
            )
            closes = sum(
                1 for line in lines
                if re.search(rf"\b{close_kw}\b", line.strip())
            )
            if opens > closes:
                return ValidationResult(
                    ...
                    error=(
                        f"Missing {close_kw} "
                        f"(found {opens} {open_kw} but {closes} {close_kw})"
                    ),
                )
        ...
        var_closes = sum(
            1 for line in lines
            if re.search(r"\bEND_VAR\b", line.strip())
        )
        if var_opens > var_closes:
            return ValidationResult(
                ...
                error=(
                    f"Missing END_VAR "
                    f"(found {var_opens} VAR blocks but {var_closes} END_VAR)"
                ),
            )
        ...
        for i, line in enumerate(lines):
            ...
            if ":=" in stripped and not stripped.endswith(";"):
                if not stripped.endswith(","):
                    warnings.append(
                        f"Line {i+1}: Assignment may be missing semicolon"
                    )
