...
def test_validate_release_artifacts_script(tmp_path):
    ...
    (bundle_dir / "training-manifest.json").write_text(
        json.dumps(
            {
    ...
    (bundle_dir / "benchmark-summary.json").write_text(
        json.dumps(
            {
                "name": "SPS",
                "dataset": "Synthetic IEC data",
    ...
    (bundle_dir / "license-manifest.json").write_text(
        json.dumps(
            {
                "release_license": "MIT",
    ...
    (bundle_dir / "environment-manifest.json").write_text(
        json.dumps(
            {
                "hardware_tier": "tier_a_rtx_3090_24gb",
                "hardware": "RTX 3090",
    ...
    (bundle_dir / "tester-attestation.json").write_text(
        json.dumps(
            {
